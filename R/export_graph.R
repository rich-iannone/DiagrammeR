...
export_graph <- function(graph,
  ...
  if (file_type == "PDF" | file_type == "pdf"){
    rsvg_pdf(charToRaw(DiagrammeRsvg::export_svg(grViz(graph$dot_code))),
             file = file_name,
             width = width,
             height = height)
  }
  ...
  if (file_type == "SVG" | file_type == "svg"){
    rsvg_svg(charToRaw(DiagrammeRsvg::export_svg(grViz(graph$dot_code))),
             file = file_name,
             width = width,
             height = height)
  }
  ...
  if (file_type == "PS" | file_type == "ps"){
    rsvg_ps(charToRaw(DiagrammeRsvg::export_svg(grViz(graph$dot_code))),
            file = file_name,
            width = width,
            height = height)
  }
