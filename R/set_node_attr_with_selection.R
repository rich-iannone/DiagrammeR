...
#' \code{dgr_graph}, set node attribute properties for
#' nodes available in a selection.
...
#' library(magrittr)
#'
#' # Create a simple graph, select the last node
#' # added, and set a value for the `type` attribute
#' graph <-
#'   create_graph() %>%
#'   add_node %>%
#'   add_node %>%
...
set_node_attr_with_selection <- function(graph,
  ...
  # Create a new graph object while retaining
  # the selection
  dgr_graph <-
    create_graph(
      nodes_df = nodes_df,
