...
#' @param from_col the name of the data frame column
#' from which edges originate.
#' @param from_attr the mapping of \code{from_col}
#' values to attributes of the graph's nodes.
#' @param to_col to_col the name of the data frame
...
#' data frame's records.
#' @param select_cols an optional character vector for
#' specifying which columns in the data frame should be
...
#' in the data frame as \code{rel} attribute values.
...
add_edges_from_df <- function(graph,
  ...
  # Verify that value for `from_col` is in the
  # data frame
  ...
  # data frame columns  by those replacement values
  ...
  # Get column numbers in data frame that are
  # edge attributes
  if (!is.null(rel_col)) {
    edge_attr_cols_df <-
  ...
  for (i in edges_created_rownames) {
    ...
    # specified column from the data frame
    if (!is.null(rel_col)) {
      graph <-
        set_edge_attr(
          x = graph,
