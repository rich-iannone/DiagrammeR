#' Combine multiple node data frames into a single
#' node data frame
#' @description Combine several node data frames in the
#' style of \code{rbind}, except, it works regardless
#' of the number and ordering of the columns.
...
combine_nodes <- function(...) {
  ...
  for (l in 1:length(data_frames)) {
    ...
    if (any(!(colnames(df2) %in% colnames(df1)))) {
      df_1_missing <-
        colnames(df2)[which(!(colnames(df2) %in% colnames(df1)))]
    } else {
      df_1_missing <- NA
    }
    if (any(!(colnames(df1) %in% colnames(df2)))) {
      df_2_missing <-
        colnames(df1)[which(!(colnames(df1) %in% colnames(df2)))]
    } else {
      df_2_missing <- NA
    }
