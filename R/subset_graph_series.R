...
#' @param subset for subsetting the graph series by graph indices (where \code{by = number}), provide a vector of those indices; when subsetting by time (where \code{by = time}), a range of times can be provided as a vector.
#' @return a numeric vector representing a count of graphs in a graph series object of type \code{dgr_graph_1D}.
