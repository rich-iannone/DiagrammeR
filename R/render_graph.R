...
#' @description Using a \code{dgr_graph} object, either render graph in the Viewer or output in various formats.
#' @param graph a \code{dgr_graph} object, created using the \code{create_graph} function.
#' @param output a string specifying the output type; \code{graph} (the default) renders the graph using the \code{grViz} function, \code{vivagraph} renders the graph using the \code{vivagraph} function, \code{DOT} outputs DOT code for the graph, and \code{SVG} provides SVG code for the rendered graph.
#' @param layout a string specifying a layout type for a \code{vivagraph} rendering of the graph, either \code{forceDirected} or \code{constant}.
