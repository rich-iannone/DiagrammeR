...
#' @description Run an R script located inside or
#' referenced from the graph
#' series object in order to migrate the state of one
#' or more contained graphs.
#' @param graph_series a graph series object of type
#' \code{dgr_graph_1D}.
#' @param script the index of the script character
#' string or path reference held in in the
#' graph series.
...
trigger_script <- function(graph_series,
  ...
  # Determine whether the script index provided
  # is within range
  ...
  # Determine whether the script called is a
  # character string or a reference to a script
  is_path_provided <-
    ifelse(grepl(".*.(r|R)$",
  ...
  if (is_path_provided == TRUE) {
    ...
    does_file_exist <-
      ifelse(
        file.exists(graph_series$series_scripts[script]),
        TRUE, FALSE)
    ...
    evaluated_series <-
      eval(parse(text = substituted_script))

    return(evaluated_series)
  }

  # Trigger script that is a character object
  # in the graph series
