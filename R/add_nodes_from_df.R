...
#' existing graph object from data in a data frame.
...
#' specifying which columns in the data frame file
#' should be imported as node attributes.
...
#' the data frame as node ID values.
#' @param type_col an option to apply a column of data
#' in the data frame as \code{type} attribute values.
#' @param label_col an option to apply a column of data
#' in the data frame as \code{label} attribute values.
...
add_nodes_from_df <- function(graph,
  ...
  if (!is.null(select_cols)){
    ...
    columns_retained <-
      which(colnames(df) %in% select_cols)

    df <- df[,columns_retained]
  }

  # If values for `drop_cols` provided, filter the df
  # columns by those named columns
  if (is.null(select_cols) & !is.null(drop_cols)){

    columns_retained <-
      which(!(colnames(df) %in% drop_cols))

    df <- df[,columns_retained]
  }

  # If values for `rename_attrs` provided, rename the
  # df columns by those replacement values
    ...
    if (!is.null(label_col)){
      graph <-
        set_node_attr(
          ...
          nodes = nodes_created[i],
          node_attr = "label",
          values = df[i, which(colnames(df) %in%
                                 label_col)])
    }
