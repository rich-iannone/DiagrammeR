...
test_that("Adding a balanced tree is possible", {
  ...
  expect_error(add_balanced_tree(
    graph,
    k = 1,
    h = 3,
    type = "a",
...
test_that("Adding a full graph is possible", {
  ...
  graph <-
    create_graph() %>%
    add_full_graph(n = 3, keep_loops = TRUE)
  ...
  graph <-
    create_graph() %>%
    add_full_graph(n = 3, keep_loops = FALSE)
  ...
  # nodes created
  expect_equal(unique(get_node_df(graph)[, 2]),
               "connected")
  ...
  # all edges created
  expect_equal(unique(graph$edges_df$rel),
               "connected_to")
  ...
  expect_equal(graph$edges_df$weight,
               c(3.3, 5.02, 4.13,
                 6.49, 6.03, 5.55))
  ...
  graph <-
    create_graph() %>%
    add_cycle(5, "cycle") %>%
    add_full_graph(n = 8, keep_loops = FALSE)
