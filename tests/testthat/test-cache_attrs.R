...
test_that("Caching node attributes is possible", {
  ...
  graph <-
    create_graph() %>%
    add_n_nodes(10) %>%
    set_node_attrs("value", rnorm(node_count(.), 5, 2)) %>%
    add_edges_w_string("1->2 1->3 2->4 2->5 3->6 3->7 4->8 4->9 5->10")

  # Cache all values from the node attribute `value`
  graph <-
    graph %>%
    cache_node_attrs(node_attr = "value",
                     name = "cached")
  ...
  expect_equivalent(as.numeric(graph$nodes_df$value),
                    graph$cache$cached)
  ...
  expect_equivalent(as.character(graph$nodes_df$value),
  ...
  expect_equivalent(as.character(graph$nodes_df$value[1:2]),
...
test_that("Caching edge attributes is possible", {
  ...
  graph <-
    ...
    add_n_nodes(10) %>%
    add_edges_w_string("1->2 1->3 2->4 2->5 3->6 3->7 4->8 4->9 5->10") %>%
    set_edge_attrs("value", rnorm(edge_count(.), 5, 2))

  # Cache all values from the edge attribute `value`
  # as a numeric vector
  graph <-
    graph %>%
    cache_edge_attrs(edge_attr = "value",
                     name = "cached",
                     mode =  "numeric")
  ...
  # after coercing to numeric
  expect_equivalent(as.numeric(graph$edges_df$value),
                    graph$cache$cached)
  ...
  # Expect the vector to be equivalent to the node
  # attribute values in the graph's internal edf
  expect_equivalent(as.character(graph$edges_df$value),
                    graph$cache$cached_2)
  ...
  expect_equivalent(as.character(graph$edges_df$value[1:2]),
                    graph$cache$cached_3)
})
...
test_that("Caching node attrs (w/ selection) is possible", {
  ...
  graph <-
    graph %>%
    cache_node_attrs_ws(node_attr = "value",
                        name = "cached",
                        mode =  "numeric")
  ...
  expect_equivalent(as.numeric(graph$nodes_df$value)[c(3, 8, 9)],
                    graph$cache$cached)
  ...
  # attribute values for certain nodes in the graph's
  # internal ndf
  expect_equivalent(as.character(graph$nodes_df$value)[c(3, 8, 9)],
                    graph$cache$cached_2)
...
test_that("Caching edge attrs (w/ selection) is possible", {
  ...
  graph <-
    graph %>%
    cache_edge_attrs_ws(edge_attr = "value",
                        name = "cached",
                        mode =  "numeric")
...
test_that("Setting a cache is possible", {
  ...
  graph_cache <-
    graph %>%
    set_cache(name = "closeness_vector",
