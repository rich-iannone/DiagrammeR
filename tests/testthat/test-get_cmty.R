...
test_that("the edge betweeness algorithm is functional", {
  ...
  expect_equal(colnames(edge_betweeness),
               c("id", "edge_btwns_group"))

  # Expect as many rows in the data frame as there
  # are nodes in the graph
  expect_equal(nrow(edge_betweeness), node_count(graph))

  # For this analysis expect 5 different groups
  expect_equal(sort(unique(edge_betweeness$edge_btwns_group)),
               c(1, 2, 3, 4, 5))

  # Expect the first column to be integer
  expect_is(edge_betweeness$id, "integer")

  # Expect the second column to be numeric
  expect_is(edge_betweeness$edge_btwns_group, "numeric")
})
...
test_that("the fast-greedy algorithm is functional", {
  ...
  # Expect specific column names in this data frame
  expect_equal(colnames(fast_greedy),
               c("id", "f_g_group"))

  # Expect as many rows in the data frame as there
  # are nodes in the graph
  expect_equal(nrow(fast_greedy), node_count(graph))

  # For this analysis expect two different groups
  # identified with labels `1` and `2`
  expect_equal(sort(unique(fast_greedy$f_g_group)),
               c(1, 2))

  # Expect the first column to be integer
  expect_is(fast_greedy$id, "integer")

  # Expect the second column to be numeric
  expect_is(fast_greedy$f_g_group, "numeric")
})
...
test_that("the leading eigenvector algorithm is functional", {
  ...
  # Expect specific column names in this data frame
  expect_equal(colnames(l_eigenvec),
               c("id", "l_eigenvec_group"))

  # Expect as many rows in the data frame as there
  # are nodes in the graph
  expect_equal(nrow(l_eigenvec), node_count(graph))

  # For this analysis expect two different groups
  # identified with labels `1` and `2`
  expect_equal(sort(unique(l_eigenvec$l_eigenvec_group)),
               c(1, 2))

  # Expect the first column to be integer
  expect_is(l_eigenvec$id, "integer")
...
test_that("the Louvain algorithm is functional", {
  ...
  # Expect specific column names in this data frame
  expect_equal(colnames(louvain),
               c("id", "louvain_group"))

  # Expect as many rows in the data frame as there
  # are nodes in the graph
  expect_equal(nrow(louvain), node_count(graph))

  # For this analysis expect two different groups
  # identified with labels `1` and `2`
  expect_equal(sort(unique(louvain$louvain_group)),
               c(1, 2))

  # Expect the first column to be integer
  expect_is(louvain$id, "integer")

  # Expect the second column to be numeric
  expect_is(louvain$louvain_group, "numeric")
})
...
test_that("the walktrap algorithm is functional", {
  ...
  # Expect specific column names in this data frame
  expect_equal(colnames(walktrap),
               c("id", "walktrap_group"))

  # Expect as many rows in the data frame as there
  # are nodes in the graph
  expect_equal(nrow(walktrap), node_count(graph))

  # For this analysis expect two different groups
  # identified with labels `1` and `2`
  expect_equal(sort(unique(walktrap$walktrap_group)),
               c(1, 2))
