...
test_that("getting a node/edge count for an empty graph is possible", {
  ...
  # will return `0`
  expect_equal(
    node_count(empty_graph, type = FALSE), 0)

  expect_equal(
    node_count(empty_graph, type = TRUE), 0)

  # Expect that an edge count of an empty graph
  # will return `0`
  expect_equal(
    edge_count(empty_graph, rel = FALSE), 0)

  expect_equal(
    edge_count(empty_graph, rel = TRUE), 0)

  # Expect that an edge count with a relationship value
  # set to any character vector will automatically
  # return `0`
  expect_equal(
    edge_count(empty_graph, rel = "rel"), 0)
})
