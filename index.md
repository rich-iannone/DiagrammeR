...
[![](reference/figures/logo.svg)](https://rich-iannone.github.io/DiagrammeR/)
...
![](reference/figures/simple_graph.png)
...
and add some nodes and edges to it. Each node gets a new integer ID upon
creation. Each edge also gets an ID starting from 1. The pipes between
functions make the whole process readable and understandable.
...
to specifically target the created nodes or edges and bind attribute
data. An additional benefit in using the helper functions (for the
node/edge aesthetic attributes especially) is that RStudio can provide
inline help on attribute names and definitions when typing `node_aes(`
or `edge_aes(` and pressing the **TAB** key.
...
calls, the new node and edge were set with a `value` node/edge data
attribute.
...
want to display your graph diagram using the
...
Now, if we view the graph with
[`render_graph()`](https://rich-iannone.github.io/DiagrammeR/reference/render_graph.md)
we get this:

![](reference/figures/e_graph.png)
...
With these selections, we can apply changes using functions that end
with `..._ws()` (with selection). As seen, node attributes could be
set/replaced with
...
or edges become the active selection) with
[`invert_selection()`](https://rich-iannone.github.io/DiagrammeR/reference/invert_selection.md),
certain nodes/edges can be removed from the active selection with the
[`deselect_nodes()`](https://rich-iannone.github.io/DiagrammeR/reference/deselect_nodes.md)/[`deselect_edges()`](https://rich-iannone.github.io/DiagrammeR/reference/deselect_edges.md),
and any selection can and should be eventually cleared with
[`clear_selection()`](https://rich-iannone.github.io/DiagrammeR/reference/clear_selection.md).
...
![](reference/figures/h_graph_2.png)
...
1.  [`create_graph()`](https://rich-iannone.github.io/DiagrammeR/reference/create_graph.md)
2.  [`add_nodes_from_table()`](https://rich-iannone.github.io/DiagrammeR/reference/add_nodes_from_table.md)
3.  [`add_edges_from_table()`](https://rich-iannone.github.io/DiagrammeR/reference/add_edges_from_table.md)
...
Inspect the graph’s internal node data frame (ndf) with
...
Inspect the graph’s internal edge data frame (edf) with
...
and
[`add_edges_from_table()`](https://rich-iannone.github.io/DiagrammeR/reference/add_edges_from_table.md).
This time, we’ll remove the auto-generated `id_external` node attribute
with the
...
and get the sums of `value_1` and `value_2` as `value_3` (for both the
nodes and the edges). Then, let’s color the nodes and edges
`forestgreen` if `value_3` is greater than `10` (`red` otherwise).
Finally, let’s display the values of `value_3` for the nodes when
rendering the graph diagram. Here we go!
...
function, where
[`system.file()`](https://rdrr.io/r/base/system.file.html) helps to
...
We can always view this property graph with the
[`render_graph()`](https://rich-iannone.github.io/DiagrammeR/reference/render_graph.md)
function:
...
![](reference/figures/graph_example_1.png)
...
([`select_edges()`](https://rich-iannone.github.io/DiagrammeR/reference/select_edges.md)
by itself selects all edges in the graph). After that, get a numeric
vector of `commits` values and then get its
[`sum()`](https://rdrr.io/r/base/sum.html) (all commits to all
projects).
...
edges. Get that vector of `commits` values with
[`get_edge_attrs_ws()`](https://rich-iannone.github.io/DiagrammeR/reference/get_edge_attrs_ws.md)
and then calculate the [`sum()`](https://rdrr.io/r/base/sum.html). This
is the total number of commits.
...
role though. In this case we’ll supply a condition in
...
met. Although there is only a single value, we’ll still use
[`sum()`](https://rdrr.io/r/base/sum.html) after
[`get_edge_attrs_ws()`](https://rich-iannone.github.io/DiagrammeR/reference/get_edge_attrs_ws.md)
(a good practice because we may not know the vector length, especially
in big graphs).
...
another
[`select_nodes()`](https://rich-iannone.github.io/DiagrammeR/reference/select_nodes.md)
call specifying `age > 32`. Importantly, have `set_op = "intersect"`
...
this with the
[`get_node_attrs_ws()`](https://rich-iannone.github.io/DiagrammeR/reference/get_node_attrs_ws.md)
function. After getting that vector, sort the names alphabetically with
the R function [`sort()`](https://rdrr.io/r/base/sort.html). Because we
get a named vector, we can use
[`unname()`](https://rdrr.io/r/base/unname.html) to not show us the
names of each vector component.
...
Traverse to the edges leading to it with
...
attribute with numerical values. Get a vector of `commits` and then get
the sum (there are `1676` commits).
...
Note that
[`add_edge()`](https://rich-iannone.github.io/DiagrammeR/reference/add_edge.md)
usually relies on node IDs in `from` and `to` when creating the new
edge. This is almost always inconvenient so we can instead use node
labels (we know they are unique in this graph) to compose the edge,
setting `use_labels = TRUE`.

The `rel` value in
[`add_edge()`](https://rich-iannone.github.io/DiagrammeR/reference/add_edge.md)
was set to `contributor` – in a property graph we always have values set
for all node `type` and edge `rel` attributes. We will set another
attribute for this edge (`commits`) by first selecting the edge (it was
the last edge made, so we can use
[`select_last_edges_created()`](https://rich-iannone.github.io/DiagrammeR/reference/select_last_edges_created.md)),
then, use
[`set_edge_attrs_ws()`](https://rich-iannone.github.io/DiagrammeR/reference/set_edge_attrs_ws.md)
and provide the attribute/value pair. Finally, clear the active
selections with
[`clear_selection()`](https://rich-iannone.github.io/DiagrammeR/reference/clear_selection.md).
...
![](reference/figures/graph_example_2.png)

Get all email addresses for contributors (but not maintainers) of the
**randomizer** and **supercalc88** projects. With
[`trav_in_edge()`](https://rich-iannone.github.io/DiagrammeR/reference/trav_in_edge.md)
we just want the `contributer` edges/commits. Once on those edges, hop
back unconditionally to the people from which the edges originate with
[`trav_out_node()`](https://rich-iannone.github.io/DiagrammeR/reference/trav_out_node.md).
Get the `email` values from those selected individuals as a sorted
character vector.
...
[`LICENSE.md`](https://rich-iannone.github.io/DiagrammeR/LICENSE.md)
